...
4. **「Erase ボタンを差し替える」** を押して、追加したペンの Erase ボタンを、まとまりごと消せるもの（[長押しの段階](grabqv.md#erase-hold)に SPLIT が加わったもの）に差し替えます。

対象から外したペンの Split ボタンが残っていると、Inspector に警告と **「不要な区切りボタンを消す」** ボタンが出ます。対象から外したペンの Erase ボタンが差し替えたままのときは、**「不要・重複した Erase ボタンを元に戻す」** ボタンで QvPen の Erase ボタンに戻せます。
